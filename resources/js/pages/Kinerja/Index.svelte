<script>
    ...
    const form = useForm({ name: '', period_start: '', period_end: '' });
...
<Layout>
    <div class="space-y-6">
        ...
        <form onsubmit={(e) => { e.preventDefault(); submit(); }} class="rounded-lg border border-gray-200 bg-white p-4 space-y-4">
            <div>
                <label class="block text-sm font-medium text-gray-700">Nama Rancangan</label>
                <input type="text" bind:value={form.name} class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm" placeholder="mis. Perjenjangan Kinerja 2026" />
